...
def deepnn(x):
  ...
  with tf.name_scope('fc1'):
    W_fc1 = weight_variable([2816, 128])
    b_fc1 = bias_variable([128])
    h_pool1_flat = tf.reshape(x, [-1,2816]) # might be useless
  ...
  with tf.name_scope('fc2'):
    ...
    h_pool2_flat = tf.reshape(h_fc1, [-1,128])
    h_fc2 = tf.nn.relu(tf.matmul(h_pool2_flat, W_fc2) + b_fc2)

  with tf.name_scope('fc3'):
    W_fc3 = weight_variable([64, 32])
    b_fc3 = bias_variable([32])
    h_pool3_flat = tf.reshape(h_fc2, [-1,64])
  ...
  with tf.name_scope('end'):
    W_fc_end = weight_variable([16, 6])
    b_fc_end = bias_variable([6])
    y_conv = tf.matmul(h_fc4, W_fc_end) + b_fc_end
...
def nnClassifier():
  ...
  # Create the model
  x = tf.placeholder(tf.float32, [None, 2816])
